...
public class NegamaxAlphaBetaPlayer extends JPanel implements IPlayerHandler {
	...
	public NegamaxAlphaBetaPlayer(ChessGame chessGame, int color, int depth) {
...
//		ChessConsole.printCurrentGameState(this.chessGame);
		...
		for (Move move : validMoves) {
			...
			int evaluationResult = negamaxAlphaBeta(this.depth, Integer.MIN_VALUE, Integer.MAX_VALUE, -1);
//			int evaluationResult = alphaBetaSearch(this.maxDepth, "");
	...
	/**
	 * evaluate current game state according to nega max algorithm with Alpha Beta pruning
	 *
	 ...
	 */		
	private int negamaxAlphaBeta(int depth, int alpha, int beta, int color) {
		if (depth <= 1
			...
			for(Move currentMove : moves){				
				...
				int v = -1 * negamaxAlphaBeta(depth - 1, -beta, -alpha, -color);
